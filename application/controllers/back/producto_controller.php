...
if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Producto_controller extends CI_Controller{
		...
		/**
	    * Muestra todos los producto en tabla */
		function index()
		{
			if($this->_veri_log()){
			$data = array('titulo' => 'Muestra de producto');
		
			$session_data = $this->session->userdata('logged_in');
			$data['perfil_id'] = $session_data['perfil_id'];
			$data['nombre'] = $session_data['nombre'];

			$dat = array('producto' => $this->producto_model->get_productos() );

			$this->load->view('front/header_view',$data);
			$this->load->view('front/menu_view2');
			$this->load->view('producto/mostrarproducto',$dat);
			$this->load->view('front/footer_view');
			}else{
			redirect('login', 'refresh'); }
		}

		function muestra_accion()
		{
			if($this->_veri_log()){
			...
			$this->load->view('front/header_view', $data);
			$this->load->view('front/menu_view2');
			$this->load->view('producto/muestracategoria_view', $dat);
			$this->load->view('front/footer_view');
			}else{
		...
		function form_agrega_producto()  	//Si se modifica, modificar (agrega_producto) tambien
		{
			...
			$this->load->view('front/header_view', $data);
			$this->load->view('front/menu_view2');
			$this->load->view('producto/Agregarproducto');
			$this->load->view('front/footer_view');
			}else{
			redirect('login', 'refresh'); }
		}
	    ...
	    */
		function agrega_producto()
		{
			...
			$this->form_validation->set_rules('descripcion', 'Descripcion', 'required|is_unique[producto.descripcion]');
		...
		function _image_upload()
		{
            ...
            if (!empty($_FILES['filename']['name']))
            {
                ...
                if ($this->upload->do_upload('filename'))
                {
                    ...
                    // Array de datos para insertar en producto
                    $data = array(
					...
					$producto = $this->producto_model->add_producto($data);
	    	...
	    	{            
	    		...
	    		if ($this->upload->do_upload('filename'))
	    		{
	    			...
	    			$data = $this->upload->data();
	                    ...
	                    // Path donde guarda el archivo..
	    			$url ="assets/img/producto/".$_FILES['filename']['name'];
		...
		*/
	    ...
	    /**
		* producto eliminados logicamente
		*/
	    function muestra_eliminados()
	    {    	
	    	if($this->_veri_log()){
	    	$data = array('titulo' => 'producto eliminados');
			$session_data = $this->session->userdata('logged_in');
			$data['perfil_id'] = $session_data['perfil_id'];
			$data['nombre'] = $session_data['nombre'];
			
			$dat = array(
		        'producto' => $this->producto_model->not_active_productos()
			);

			$this->load->view('front/header_view', $data);
			$this->load->view('front/menu_view2');
			$this->load->view('producto/eliminarmostrar', $dat);
			$this->load->view('front/footer_view');
			}else{
			redirect('login', 'refresh');}
		}
 	...
 	function listar_ventas()
	    { 
			...
			$this->load->view('parte/usuario/muestraventas',$dat);
			$this->load->view('front/footer_view');
            }else{
			redirect('login', 'refresh');
            }
        ...
        function muestra_detalle($id)
		{
			...
			$this->load->view('front/header_view', $data);
			$this->load->view('front/menu_view2', $data);
			$this->load->view('parte/usuario/muestradetalle', $dat);
			$this->load->view('front/footer_view');
            }else{
			redirect('login', 'refresh');
            }
